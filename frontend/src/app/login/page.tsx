...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    if (!supabase) {
      toast.error('Supabase is not configured. Please add your keys to frontend/.env.local');
      return;
    }
    try {
      setIsLoading(true);
    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      ...
      >
        <div className="flex flex-col items-center text-center space-y-6">
          ...
          {!isConfigured && (
            <div className="w-full p-4 rounded-xl bg-yellow-500/10 border border-yellow-500/30 text-left">
              <div className="flex items-start gap-3">
                <AlertTriangle className="w-5 h-5 text-yellow-400 mt-0.5 shrink-0" />
                <div>
                  <p className="text-sm font-semibold text-yellow-300">Supabase Not Configured</p>
                  <p className="text-xs text-yellow-200/70 mt-1">
                    Google Sign-In requires Supabase. Add your keys to <code className="bg-white/10 px-1 rounded">frontend/.env.local</code> to enable it.
                  </p>
                </div>
              </div>
            </div>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 px-6 py-4 rounded-xl bg-white text-black font-semibold hover:bg-gray-100 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
          >
            {isLoading ? (
